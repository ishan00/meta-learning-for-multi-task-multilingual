...
parser.add_argument('--qa_batch_size', type=int, default=4, help='batch size')
parser.add_argument('--sc_batch_size', type=int, default=16, help='batch size')
parser.add_argument('--tc_batch_size', type=int, default=16, help='batch size')
parser.add_argument('--rc_batch_size', type=int, default=4, help='batch size')
parser.add_argument('--pa_batch_size', type=int, default=16, help='batch size')

parser.add_argument('--task_per_queue', type=int, default=8, help='')
parser.add_argument('--update_step', type=int, default=4, help='')
...
parser.add_argument('--dds_lr', type=float,default=1e-3)
...
loss_tasks_weights = {k:torch.nn.Parameter(torch.zeros(1,device = DEVICE)) for k in list_of_tasks}
sigmas = [loss_tasks_weights[key] for key in loss_tasks_weights.keys()]
...
optimizer_grouped_parameters = [
	...
	{
		"params": [loss_tasks_weights[k] for k in loss_tasks_weights.keys()],
		"lr": args.sigma_lr
	}
]
...
def multiDDSUpdate(init_p, list_of_tasks, curr_step, total_steps):
	...
	if (curr_step+1)%args.update_dds == 0:
		...
		for task_idx, task in enumerate(list_of_tasks):
			reward = 0
			batch = get_batch(list_of_psi_train_iters[task], psi_train_dataloaders[task])
		...
		p = np.array([torch.exp(psis[x]).item() for x in list_of_tasks])
		p = p/np.sum(p)
		print([(task,p[idx]) for idx,task in enumerate(list_of_tasks)])
		print('======================================================================')

	return p
# def UniformTaskDecaySampler():
# 	task_set = set()
# 	for tk in list_of_tasks:
# 		task_set.add(tk[:2])
# 	p = [1/len(task_set) if 'en' in y else 0 for y in list_of_tasks]
# 	sampler = iter(Sampler(p, steps, shiftToLowResource))
# 	return sampler

# if args.sampler == 'uniform_task_type': sampler = UniformTaskTypeSampler()
# elif args.sampler == 'uniform_batch': sampler = UniformBatchSampler()
# elif args.sampler == 'uniform_task_decay': sampler = UniformTaskDecaySampler()
...
def metastep(model, queue):
	...
	for i in range(n):
		for k in range(args.update_step):
			...
			loss = output[0].mean()
			if args.sigma:
				loss = loss/torch.exp(loss_tasks_weights[queue[i]['task']][0])
				loss = loss + loss_tasks_weights[queue[i]['task']][0]/2
			loss.backward()
...
def evaluateMeta(model):
	...
	for task in list_of_tasks:
		...
		loss_dict[task] = loss
		if args.sigma:
			total_loss += (loss/torch.exp(loss_tasks_weights[task][0]).item() + loss_tasks_weights[task][0].item()/2)*len(train_dataloaders[task])/sum([len(train_dataloaders[x]) for x in list_of_tasks])
		else:
			total_loss += loss
...
def main():
	...
	logger['val_f1'] = []
	logger['sigmas'] = {k:[] for k in list_of_tasks}
	logger['train_loss'] = []
	...
	try:
		...
		for j,metabatch in enumerate(sampler):
			...
			for task in list_of_tasks:
				logger['sigmas'][task].append(loss_tasks_weights[task].item())
			...
			if (j + 1) % args.log_interval == 0:
				...
				if args.sigma:
					for task in list_of_tasks:
						print ('sigma %s %f'%(task,loss_tasks_weights[task].item()),end=',')
