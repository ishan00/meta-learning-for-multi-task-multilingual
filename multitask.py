...
parser.add_argument('--task_per_queue', type=int, default=1, help='')
...
parser.add_argument('--log_interval', type=int, default=10000, help='Print after every log_interval batches')
...
steps = args.meta_epochs * sum([len(train_dataloaders[x]) for x in list_of_tasks]) 
...
class Sampler:
	...
	def __next__(self):
		...
		l = {'task':tasks[0],'data':get_batch(self.list_of_iters[tasks[0]], train_dataloaders[tasks[0]])}
		return l
