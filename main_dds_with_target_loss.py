...
def multiDDSUpdate(init_p, list_of_tasks, curr_step, total_steps):
	...
	if (curr_step+1)%args.update_dds == 0:
		...
		with torch.no_grad():
			...
			dev_loss_init = output[0].mean()
		del output
		#dev_loss_init = evaluate(model, args.target_task, dev_dataloaders[args.target_task])
		for task in list_of_tasks:
			...
			with torch.no_grad():
				output = model.forward(args.target_task, dev_batch)
				dev_loss_final = output[0].mean()
			#dev_loss_final = evaluate(model, args.target_task, dev_dataloaders[args.target_task])
			reward = dev_loss_init - dev_loss_final
			print (task, reward)
			rewards[task] = reward
			for idx,param in enumerate(model.parameters()):
				param.data = old_vars_dds[idx].data.clone()
#			optim.load_state_dict(init_state_dict)
