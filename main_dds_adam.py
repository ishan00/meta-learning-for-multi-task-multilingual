import torch, os, random, time, argparse, logging, pickle, json, copy, warnings, sys
import numpy as np
...
psis = torch.nn.Linear(len(list_of_tasks),1).to(DEVICE)
for p in psis.parameters():
	p.data.fill_(0.)

data_optimizer = torch.optim.Adam(lr=args.dds_lr,params=psis.parameters())

l = 100
lan_dist = np.array([len(train_dataloaders[y])*1.0/sum([len(train_dataloaders[x]) for x in list_of_tasks]) for y in list_of_tasks])
target = torch.FloatTensor(lan_dist).view(1,-1)
feature = torch.ones(1,len(list_of_tasks))
target = target.cuda()
feature = feature.cuda()
while l > 0.000001:
	a_logits = psis.weight * feature
...
def multiDDSUpdate(init_p, list_of_tasks, curr_step, total_steps):
	...
	with torch.no_grad():
		a_logits = psis.weight * feature
		prob = torch.nn.functional.softmax(a_logits, dim=-1)
		p = np.array([i for i in prob.data.view(-1).cpu().numpy()])
	...
	if (curr_step+1)%args.update_dds == 0:
		...
		grad_scale = torch.FloatTensor([rewards[train_task] for train_task in list_of_tasks]).view(1,-1)
		mean = torch.mean(grad_scale)
# 		std = torch.std(torch.tensor([rewards[train_task] for train_task in list_of_tasks]))
		for train_task in list_of_tasks:
			rewards[train_task] = (rewards[train_task] - mean)# / std
		grad_scale = (grad_scale - mean).cuda()
		feature = torch.ones(1,len(list_of_tasks))
		feature = feature.cuda()
		a_logits = psis.weight * feature
		...
		with torch.no_grad():
			...
			prob = torch.nn.functional.softmax(a_logits, dim=-1)
			p = np.array([i for i in prob.data.view(-1).cpu().numpy()])
		print(['{:s},{:6.4f},{:6.4f},{:6.4f}'.format(task,p[idx],rewards[task].item(),psis.weight[0,idx].item()) for idx,task in enumerate(list_of_tasks)])
		logger['psis'].append(['{:s},{:6.4f},{:6.4f},{:6.4f}'.format(task,p[idx],rewards[task].item(),psis.weight[0,idx].item()) for idx,task in enumerate(list_of_tasks)])
		print('======================================================================')
